...
waypoints(1,:) = [0 0 0 0];
waypoints(2,:) = [0 0 1 atan2(gates(1,1), gates(1,2))];
for i=0:50
    ...
    % heading
    waypoints(i*3 + 3,4) = atan2(gates(currentGate,1) - waypoints(i*3 + 3,1), gates(currentGate,2) - waypoints(i*3 + 3,2)); % before the gate
    waypoints(i*3 + 4,4) = atan2(gates(nextGate,1) - waypoints(i*3 + 4,1), gates(nextGate,2) - waypoints(i*3 + 4,2)); % at the gate center
    waypoints(i*3 + 5,4) = atan2(gates(nextGate,1) - waypoints(i*3 + 5,1), gates(nextGate,2) - waypoints(i*3 + 5,2)); % after the gate   
end
